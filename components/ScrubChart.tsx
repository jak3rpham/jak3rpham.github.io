...
export function ScrubChart({
  ...
  return (
    <div ref={ref} className="rounded-[12px] border border-panel-border bg-panel p-6 backdrop-blur-md md:p-8">
      <div className="mb-5 border-b border-rule pb-3 font-mono text-[0.64rem] uppercase tracking-[0.1em] text-sand">{barLabel}</div>
      ...
      <div className="mt-2 flex justify-between font-mono text-[0.58rem] uppercase tracking-[0.08em] text-sand">
        {xLabels.map((x) => (
          <span key={x}>{x}</span>
        ))}
      </div>
      <div className="mt-3 font-mono text-[0.64rem] leading-relaxed text-sand">{caption}</div>
    </div>
  );
}
